...
function ExplorerInner() {
  ...
  useEffect(() => {
    ...
    // Intentionally no deps - we only want to register once on mount
  }, []);
  ...
  useEffect(() => {
    const off = eventBus.on("theme:changed", (theme: string) => {
      // Global behavior on theme change if needed
    });
    return () => off();
  }, []);
  ...
  function startAutoLoadSequence() {
    ...
    let idx = 0;
    const tickDelay = Math.max(200, steps[0].delay || 200);
  ...
  if (isInitialLoading) {
    return (
      <div className={`fixed inset-0 z-50 flex items-center justify-center ${isDark ? 'glass' : 'bg-white/95 backdrop-blur-xl'}`}>
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden top-[5rem] lg:top-[8rem] ">
      ...
      <div className={`fixed inset-0 -z-20 pointer-events-none ${isDark ? 'bg-gradient-to-br from-black via-gray-900 to-black' : 'bg-gradient-to-br from-gray-50 via-white to-gray-100'}`}>
        <div className={`absolute top-1/4 -left-20 w-96 h-96 ${isDark ? 'bg-[var(--accent)]/5' : 'bg-[var(--accent)]/3'} rounded-full blur-3xl`} />
        <div className={`absolute bottom-1/4 -right-20 w-96 h-96 ${isDark ? 'bg-[var(--accent-dark)]/5' : 'bg-[var(--accent-dark)]/3'} rounded-full blur-3xl`} />
      </div>

      {/* Blocker overlay */}
      {showBlocker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8">
          <div className={`absolute inset-0 ${isDark ? 'bg-black/80 backdrop-blur-xl' : 'bg-white/80 backdrop-blur-xl'}`} />
      ...
      {autoLoading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className={`absolute inset-0 ${isDark ? 'bg-black/90 backdrop-blur-2xl' : 'bg-white/95 backdrop-blur-2xl'}`} />
          <div className={`relative z-10 w-full max-w-md p-8 rounded-2xl ${isDark ? 'glass-heavy' : 'bg-white/90 border border-gray-300/30'} text-center space-y-6`}>
      ...
      <div className="min-w-full px-4 md:px-6 lg:px-8 pb-8 space-y-12">
        ...
        <section className={`mt-4 rounded-2xl ${isDark ? 'glass-heavy border border-white/10' : 'bg-white/90 backdrop-blur-xl border border-gray-300/30'}`}>
          <div className="p-6">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                {/* Price Display */}
                <div className={`px-4 py-2.5 rounded-xl ${isDark ? 'bg-black/30 border border-white/10' : 'bg-white/50 border border-gray-300'}`}>
                  <div className="flex items-center gap-2">
                    <div className="text-sm">
                      ZEC <span className="font-bold">${price?.usd?.toFixed(4) ?? "—"}</span>
                    </div>
                    {price?.change24h !== undefined && price?.change24h !== null && (
                      <div className={`text-xs px-1.5 py-0.5 rounded ${price.change24h >= 0 ? 'bg-green-500/10 text-green-600' : 'bg-red-500/10 text-red-600'}`}>
                ...
                <button
                  ...
                  className={`p-2.5 rounded-xl ${isDark ? 'bg-black/30 border border-white/10 hover:bg-white/5' : 'bg-white/50 border border-gray-300 hover:bg-gray-100'} transition-all duration-300`}
                >
                  <RefreshCw className={`w-4 h-4 ${priceLoading ? 'animate-spin' : ''}`} />
                </button>

                {/* Wallet Status */}
                <div className={`px-3 py-2 rounded-xl ${isDark ? 'bg-black/30 border border-white/10' : 'bg-white/50 border border-gray-300'}`}>
                  <div className="flex items-center gap-2">
            ...
            {/* Progress Bar */}
            {explorer?.progress && (
              <div className={`mt-6 p-4 rounded-xl ${isDark ? 'bg-black/30 border border-white/10' : 'bg-white/50 border border-gray-300'}`}>
                ...
                <div className={`w-full rounded-full h-1.5 overflow-hidden ${isDark ? 'bg-white/10' : 'bg-gray-300'}`}>
                  <div
        ...
        <div className="grid grid-cols-1 xl:grid-cols-1 gap-8">
          ...
          <div className="xl:col-span-1 space-y-8 ">
            {/* Dashboard Stats */}
            <div className={`rounded-2xl ${isDark ? 'glass-heavy border border-white/10' : 'bg-white/90 backdrop-blur-xl border border-gray-300/30'}`}>
              <div className="p-6">
            ...
            <div className={`rounded-2xl ${isDark ? 'glass-heavy border border-white/10' : 'bg-white/90 backdrop-blur-xl border border-gray-300/30'}`}>
              <div className="p-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button className={`px-3 py-1.5 rounded-lg ${isDark ? 'bg-black/30 hover:bg-white/5' : 'bg-gray-100 hover:bg-gray-200'} transition-colors text-sm flex items-center gap-2`}>
                ...
                <div className="relative w-full overflow-hidden rounded-xl ">
                  <div className={`absolute inset-0 ${isDark ? 'bg-black/20' : 'bg-gray-100/50'}`} />
            ...
            <div className={`rounded-2xl ${isDark ? 'glass-heavy border border-white/10' : 'bg-white/90 backdrop-blur-xl border border-gray-300/30'}`}>
              <div className="p-6">
                ...
                <div className="space-y-3">
                  <div className={`flex items-center justify-between p-2.5 rounded-lg ${isDark ? 'bg-black/30' : 'bg-gray-100/50'}`}>
                    <div className="flex items-center gap-2">
